...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['checkout'])) {
        ...
        if(is_numeric($row['MAX(orderId)'])){
            $new_value = intval($row['MAX(orderId)']) + 1;
        }

        if (password_verify($password, $passRow['password'])){ 
            $sql = "INSERT INTO `orders` (`orderId`,`userId`, `address`, `zipCode`, `phoneNo`, `amount`, `paymentMode`, `orderStatus`, `orderDate`) VALUES ('$new_value','$userId', '$address', '$zipcode', '$phone', '$amount', '0', '0', current_timestamp())";
            $result = mysqli_query($conn, $sql);
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    ...
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `pizzaId`, `itemQuantity`) VALUES ('$new_value', '$pizzaId', '$itemQuantity')";
                    $itemResult = mysqli_query($conn, $itemSql);
                }
                $deletesql = "DELETE FROM `viewcart` WHERE `userId`='$userId'";   
                $deleteresult = mysqli_query($conn, $deletesql);
                echo '<script>alert("Thanks for ordering with us. Your order id is ' .$new_value. '.");
                    window.location.href="http://localhost/DBMSmini/index.php";  
                    </script>';
                    exit();
            }
        } 
